...
const {getSearchResultsCount, getSearchResults} = require('../data/search2');
...
function getSearchResultsAndRender(req, res) {
    ...
    return function(rowCountData) {
        ...
        return getSearchResults(req.session.userInput).then(searchResult => {
            return res.render('search/results2', parseResultsPageData(req, rowCount, searchResult, currentPage));
        }).catch(error => {
